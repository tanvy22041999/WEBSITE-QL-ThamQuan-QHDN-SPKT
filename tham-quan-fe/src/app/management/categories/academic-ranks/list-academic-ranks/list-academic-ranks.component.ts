...
export class ListAcademicRanksComponent implements OnInit {
  ...
  breadcrumbObj: BreadCrumb = new BreadCrumb({
    heading: this.langData[this.langCode].HOC_HAM,
    listBreadcrumb: [{
  ...
  getDataPaging(): void {
    this.spinner.show();
    this.hocHamSvc.getAllPaging(
      this.listHocHam.currentPage - 1,
      this.listHocHam.limit,
      this.searchValue)
      .subscribe(res => {
        this.listHocHam.currentPage = res.pageable.pageNumber + 1;
  ...
  openModal(template: TemplateRef<unknown>, data?: HocHam): void {
    if (data) {
    ...
    this.nzModalSvc.create({
      ...
      nzTitle: (data ? this.langData[this.langCode].CHINH_SUA_TITLE :
        this.langData[this.langCode].THEM_MOI_TITLE) + this.langData[this.langCode].HOC_HAM,
      nzContent: template,
      nzFooter: null,
  ...
  pageChange(page: Paginate<HocHam>): void {
