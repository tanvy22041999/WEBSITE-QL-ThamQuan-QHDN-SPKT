...
import { CauHoiKhaoSatDoanhNghiep, KetQuaKhaoSatDoanhNghiepDto } from 'src/app/core/models/categories/cau-hoi-khao-sat-doanh-nghiep';
...
export class FormToursSurveyComponent implements OnInit {
  ...
  listCauHoi: CauHoiKhaoSatDoanhNghiep[] = [];
    ...
    if (this.setCauHoiBatBuocChuaTraLoiId.size) {
      ...
      this.ketQuaKhaoSatSvc.create(finalResult)
        .subscribe(() => {
          ...
          this.router.navigate(['/work-business', 'survey']);
          this.spinner.hide();
        }, () => this.spinner.hide());
